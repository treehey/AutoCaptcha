...
- 运行策略：分差低于 `0.40` 时，纯标点模式转人工；自动点击模式最多换图重试 `5` 次。自动点击默认关闭，且要求原图为 `250x120`、顶部候选区相对固定背景的平均 RGB 残差不超过 `12`。
...
- 真实页面仍需验证扩展派发事件是否被 `event.isTrusted` 拒绝，以及验证码失败的可靠反馈。未验证前，默认保持自动点击关闭。
