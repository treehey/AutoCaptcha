...
- CourseProvider：生产环境当前扫描入口；默认每轮串行查询最多 12 个目标，正常规模任务首轮全查，超出安全上限才按优先级分批；精确目标保存独立于教学班类别的 `queryScope`，查询按课程分类路由；发现可用候选后通过精确课程号物化 DOM，接口空结果则与当前页面可见 DOM 做一次保守校验，不会丢失已满或已选课程；
...
- [x] 查询保持串行和硬上限，但默认覆盖 12 个目标，使常见任务首轮全查，超出上限才分批；
...
- [x] 网络、服务端和限流使用带抖动的指数退避，并持久化恢复时间；
...
- [x] 页面关闭按钮和 Popup 持久开关可整体隐藏课程雷达、行内加入按钮和布局增强，不影响后台任务；
