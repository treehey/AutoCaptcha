<div align="center">
  ...
  **南京大学统一身份认证 · 自动登录助手**
  ...
  ![Version](https://img.shields.io/badge/version-3.0.0-634798?style=flat-square)
...
| **🌐 多网站支持** | 适配多个南大系统（统一身份认证、选课系统等），持续扩展中。 |
...
1.  **配置账号信息**：
    ...
    * 在弹出的面板中：
      - **统一身份认证**：输入您的学号/工号和密码
      - **选课系统**：在"选课系统"区域输入选课系统的学号和密码
    * 根据需要开启或关闭相应的开关选项。
    * 点击"保存"按钮。

2.  **自动登录**：
    * 访问对应的南大系统登录页面。
    * 插件将自动识别验证码、填充账号密码（根据你的设置）。
    * 对于**统一身份认证系统**：自动识别、自动填充、自动点击登录按钮
    * 对于**选课系统**：自动填充账号密码、自动识别点击式验证码、自动点击登录
    * 在识别过程中，页面中央会显示一个"正在处理..."的加载动画。
...
### 2. **选课系统** (v3.0新增)
   ...
   - 技术：中文OCR识别 + 字符位置定位 + 模拟点击
   - 说明：使用中文训练数据(`chi_sim.traineddata`)识别验证码中的字符，自动计算点击坐标并模拟点击
